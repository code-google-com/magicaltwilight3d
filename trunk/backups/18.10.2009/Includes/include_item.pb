...
Procedure Item_AddWaffe ( *itemID.ITEM , angriff , Reichweite, Waffenart )
...
Procedure Item_Add (name$, x.f , y.f , z.f , goldwert , ItemArt.w , Betrag.w , Gewicht.f , pfad.s , texture1.s , texture2.s , MaterialType.i ) ; betrag = z.b. lebensbonus, 
  ...
  *itemID = AddElement( item()) ; deswegen pointer, damit bei evtl multithreading kein Fehler.
  ...
  If *itemID 
     ...
     *itemID            \art           = *itemidArt   ; siehe #*itemid_art_..)
     *itemID            \anz_Mesh_ID   = anz_addmesh     ( pfad , x , y,  z , texture1 , MaterialType , texture2 , 0 , #anz_col_box , #anz_ColType_movable ) ; bei collision fällt das ding immer durch'n boden..???.. evtl später eigene  collision einbauen.
     *anz_mesh          =  *itemID     \anz_Mesh_ID 
     *anz_mesh          \itemID       = *itemID
     
     ;{ Alle wichtigen Daten von anz_mesh sammeln. für späteres Drop*itemid.
   ...
   Protected *anz_mesh.anz_mesh 
	...
	If AddElement( item())
      ...
      If item_predefined(d)\exist 
         ...
         item           () \anz_Mesh_ID = anz_addmesh     ( item_predefined(d)\pfad , x , y,  z , item_predefined(d)\texture1 , item_predefined(d)\MaterialType , item_predefined(d)\texture2 , 0 , #anz_col_box , #anz_ColType_movable ) 
...
Procedure Item_FocusItem_Reset ()  ; überprüft das Fokus-Item
   ...
   Item_Examine_Reset(1 , #anz_meter * 3) ; alle umliegenden Items überprüfen
      ...
      While Item_examine_Next()
         ...
         IrrGetNodeAbsolutePosition ( item_examine_GetIrrNode( )                  ,@pix , @piy , @piz ) ; item position.. in dem fall absolute, weils um abstand item/spieler geht.
         ...
         If math_distance3d         ( px,py,pz,pix,piy,piz) < #meter*1.7 ; wenn der abstand von item zu spieler kleiner 1.7 m ist .. ;math_square_distance2d  ( 0 , pz , 0 , piz )     < #meter * 2 And math_square_distance2d( 0 , pz , 0 , piz) > - 1.3* #meter  ; Item darf max. 2m über und 1m unterhalb von spieler sein.
             ...
             EndIf 
             Debug "itemwinkel: "  + StrF( ItemWinkel , 1)
...
Procedure Item_Examine_Reset( only_they_in_view.b = 1 , max_distance.f = 3*#meter  ) ; Überprüft, ob Nodes in der Nähe sind (bzw. alle nodes überhaupt z.b. zum material setzen)
     Protected x.i,y.i,z.i,x1.f , y1.f , z1.f ,  *p_obj.anz_Object3d , ObjectArt.w, px.f, py.f, pz.f , irr_obj.i , rasterx.i , rastery.i , rasterz.i , *anz_mesh.anz_mesh    ; wenn OnlyItems1_OnlyWesen2 = 1, dann nur items suchen, wenn =2, dann nur wesen.
      ...
      IrrGetNodePosition( spi_GetPlayerNode( spi_getcurrentplayer()) , @x1 , @y1 , @z1)   ; prüft die Position des spielers. weil aus der include_spieler.pb -> befehle beginnen mit "spi_"
...
Procedure item_drop_item ( *itemID.ITEM)
   Protected *anz_mesh.anz_mesh 
...
; FoldLines=0010002F0031004600480051005500570059009D00A100DC00DE00E000E200E4
; FoldLines=00E600E800EA00EC00EE00F200F400F600FA0104010601100112011C01440146
; FoldLines=0148014A
; Build=0
